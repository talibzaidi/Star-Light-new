...
If Database <> "" Then   ' i.e. if a database has been chosen.
    ...
    If Database = "OptedInMailingList.mdb" Then
        ...
        ' Display ~ all recipient records: (I cannot get this to work.)
        'DisplayRecipientsSQL = "SELECT Last(OrderID) AS OrderID2, FName, LName, Email, OptInToEmailings, WeEmail FROM Orders " & _
        '                "WHERE Not IsNull(OrderID2) AND OptInToEmailings = True AND WeEmail = True " & _
        '                "GROUP BY FName, LName, Email, OptInToEmailings, WeEmail " & _
        '                "ORDER BY OrderID2"
	                    '"GROUP BY OrderID, FName, LName, Email, OptInToEmailings, WeEmail " & _
                        '"ORDER BY OrderID"

        ' Display ~ all recipient records:
        DisplayRecipientsSQL = "SELECT Last(OrderID) AS OrderID, FName, LName, Email, OptInToEmailings, WeEmail FROM Orders " & _
...
<table align=center border=1 cellpadding=5>
    <FORM name="MinAndMaxIDForm">
        <% 'If Database = "PurchasedMailingList1.Tiny.mdb" Then   ' I cannot get this min and max Record ID stuff to work for Database = "OptedInMailingList.mdb"  %>
        <tr />
            <td>
