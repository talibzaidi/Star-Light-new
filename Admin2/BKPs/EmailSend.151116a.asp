...
If (DBtoUse = 1) Then
    ...
    If EmailToSend = "" Then
        EmailToSend = 10
    End If
ElseIf (DBtoUse = 2) OR (DBtoUse = "") Then
...
If Database = "OptedInMailingList.mdb" Then
    ...
    ' Display ~ all records:
    DisplaySQL = "SELECT Last(OrderID), FName, LName, Email, OptInToEmailings, WeEmail FROM Orders " & _
...
ElseIf Database = "PurchasedMailingList1.Tiny.mdb" Then
    ...
    ' Display all records:
    DisplaySQL    = "SELECT ID, [Executive Email] AS Email, State, [Last Name] AS LName, [First Name] AS FName FROM CompanyProspects " & _
                    "ORDER BY ID"  
...
Response.Write "<br><br>DisplaySQL = "      & DisplaySQL
...
Response.Write "<table width='1000' align='center'>"
Response.Write "<tr><td>"
Response.Write "DisplaySQL = "      & DisplaySQL
...
If (MinID = "") AND (MaxID = "") Then
    Response.Write "<br>Please choose a Min and Max Record ID"
End If


'rsRecipients.Open RecipientsSQL, Conn, adOpenStatic, adLockOptimistic, adCmdText 
rsDisplayRecords.Open DisplaySQL, Conn, adOpenStatic, adLockOptimistic, adCmdText
