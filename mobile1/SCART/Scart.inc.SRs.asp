...
' [12/1/20, BN]  
' Google Search Console finds that the following URL causes a 5xx server error, apparently bc the item 010-00756-10 no longer exists in the db.
' https://www.starlite-intl.com/mobile1/scart/scart.asp?action=add&item=010-00756-10&count=1&sid=0&Area=&sar=Special 
' That URL tries to add item/product 010-00756-10 to the Shopping Cart. The simplest way to avoid the 5xx error would be to modify my Sub AddItemToCart 
' below to first test if the item is in db and if it isn't then AddItemToCart should just do nothing and not try to add item to the Cart. 
' If that change works, then the URL will no longer cause a 5xx error.

' Or just have Sani/Sanction never actually delete a product from the db but rather just have Sani/Sanction check the "Deleted" column 
' for a product to indicate that it has been virtually, but not actually, deleted. (Ditto for categories and subcategories.)

' BTW, argument iItemCount is the quantity of iItemID to be added to Cart. 
Sub AddItemToCart(iItemID, iItemCount)
    iItemID = Trim(iItemID)     				' Added 9/23/11.
    'Response.Write "<br>iItemID = " & iItemID  & "... mobile1 ... SCART.INC.SRS.ASP ... iItemCount = " & iItemCount 
    ' 12/1/2020: To test if item is in the database:
                  ' Using the connection string method instead of the DSN method.
                  Set ConnProducts = Server.CreateObject("ADODB.Connection")
		  'Response.Write "<br>Session('ConnectionString') = " & Session("ConnectionString") & "<br>"
                  ConnProducts.Open Session("ConnectionString")
                  SQLstring = "SELECT * FROM Product WHERE ITEMID LIKE '" + iItemID +   "' " 
		  'Response.Write "<br>SQLstring = " & SQLstring & "<br>"
                  Set RSproducts = Server.CreateObject("ADODB.Recordset")
                  'RXS.Open SQLstring, ConnProducts, adOpenStatic, adLockOptimistic, adCmdText 
                  RSproducts.Open SQLstring, ConnProducts, 3, 3, &H0001 

' [11/26/20] Re following if-then-else see https://p2p.wrox.com/classic-asp-basics/93350-why-use-bof-eof.html
' [12/1/20] I added the following if-then.
if RSproducts.EOF AND RSproducts.BOF then	
	Response.Write "<div style='font-family:verdana; font-size:20px; text-align:center'>"
	Response.Write "<br><br><p>Product " & iItemID & " was not found in site database</p>"	
	Response.Write "<br><a href='https://www.starlite-intl.com/mobile1'>Go to home page</a>"
	Response.Write "</div>"
	Response.End
end if
     	       	  'Response.Write "RSProducts (ITEMID) = " & RSProducts("ITEMID") & "<br>"
    								' The above Write(s) in AddItemToCart CAN display on the website when adding a product to Cart. 
								' Use that for debugging. Otherwise, comment out the Writes.

    If dictCart.Exists(iItemID) Then   				' iItemID is already in Cart.
        dictCart(iItemID) = dictCart(iItemID) + iItemCount	' This supposedly updates the quantity of iItemID in Cart (though the syntax is a bit strange).
    Else							' iItemID is not yet in Cart.
        dictCart.Add iItemID, iItemCount			' Add iItemID, with its quantity iItemCount, to Cart.
    End If
%>
	...
	<table Border="0" CellPadding="7" CellSpacing="0" align="left">
		<tr bgcolor="#DDDDDD">
			...
			<!-- <td align=center><font face="tahoma" size="2"><b>Add?</b></font></td> 
                [BN, 11/26/17] Deleting the Accessories column, just to help keep cart width narrow 
                enough to fit on smart phone screens; the code itself in this column was probably
                working fine, like it does in the cart on the main site. -->
		</tr>
		...
		<tr>
			...
			<td bgcolor="#EEEEEE">
				...
				'rsThisProduct.Open AccessoriesSQL, "DSN=STAREC1" , 1, 4
                  ...
                  Set ConnThisProduct = Server.CreateObject("ADODB.Connection")
                  ConnThisProduct.Open Session("ConnectionString")
                  SQLstring = "SELECT HasAccessories, SID FROM Product WHERE  PID = " + CStr(bParameters(5)) +   " "
                  Set rsThisProduct = Server.CreateObject("ADODB.Recordset")
                  'rsThisProduct.Open SQLstring, ConnThisProduct, adOpenStatic, adLockOptimistic, adCmdText 
                  rsThisProduct.Open SQLstring, ConnThisProduct, 3, 3, &H0001 
                ...
                ' 11/9/15: Using the connection string method instead of the DSN method above.
                Set ConnSubArea = Server.CreateObject("ADODB.Connection")
                ConnSubArea.Open Session("ConnectionString")
                SQLstring = "SELECT * FROM SubArea WHERE SID = " & SubAreaID  &   " "
                Set rsSubArea = Server.CreateObject("ADODB.Recordset")
                'rsSubArea.Open SQLstring, ConnSubArea, adOpenStatic, adLockOptimistic, adCmdText 
                rsSubArea.Open SQLstring, ConnSubArea, 3, 3, &H0001 
		...
		<tr>
			<td colspan="5" ALIGN="Center" bgcolor="#DDDDDD">
				<font face="tahoma" size="2">
                    <b></b><a href="https://www.starlite-intl.com/mobile1/index.asp"><b>Continue Shopping?</b></a>
				</font>
			</td>
		</tr>    
		...
		<tr>
			<td colspan="6">
				<center>
					...
					<br><big><span style="font-family: helvetica,arial,sans-serif;">Order online or by phone at: 1-800-387-8535
                        <br><br>
                        <a href="mailto:sales@starlite-intl.com">Contact us</a> 
...
Sub PlaceOrder()
    ...
    %>
        ...
        <% If aParameters(7) = TRUE Then osize = 15.00 End If %> 
...
' 11/10/15: Same as Function asGetItemParameters(iItemID) in EmailBuild.asp? So redundant there?
' 12/1/20: The following function asGetItemParameters(iItemID) is called by ShowItemsInCart() and PlaceOrder() in this file.
Function asGetItemParameters(iItemID)
    ...
    Else
                ...
                Set ConnNonUSA = Server.CreateObject("ADODB.Connection")
                ConnNonUSA.Open Session("ConnectionString")
                SQLstring = "SELECT *,  Rates.ExchangeRate1 AS Exch, Rates.Freight AS Freight FROM Product, Rates WHERE ITEMID LIKE '" + iItemID +   "' "
                Set RXS = Server.CreateObject("ADODB.Recordset")
                'RXS.Open SQLstring, ConnNonUSA, adOpenStatic, adLockOptimistic, adCmdText 
                RXS.Open SQLstring, ConnNonUSA, 3, 3, &H0001 
