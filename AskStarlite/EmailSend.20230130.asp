...
<%
' [BN, 12/23/17] 
' This file AskStarlite/EmailSend.asp is POST-ed to by the form in Sani's Contact Us page of file Misc2/contact.asp
' It is not the same as Admin2/EmailSend.asp, which is for sending mass-mailings to Sani's opted-in customers or those in his purchased mailing list. 
...
' [12/6/20, BN:] This section was copied from main site version of this EmailSend.asp page.
' Just an experiment, to use the Contact Us page of the mobile site to email myself (and Sani) the user's HTTP_USER_AGENT string, together with
' the other fields of the Contact Us page. 

' Assuming the user filled out that mobile Contact Us page using a mobile device (after being automatically redirected from the home page of the
' main site), then the email sent by the present page will show the HTTP_USER_AGENT for the **mobile** device being used. 

' But, if I fill out the mobile Contact Us page using a non-mobile device such as my laptop (after pressing the "Mobile Site" button at the 
' bottom of the home page of the main site, to take me to mobile site), then the email sent by the present page will show the HTTP_USER_AGENT for 
' the *laptop* being used.

' Note that as of [12/6/20] a mobile Adroid device is the only device that gets automatically redirected to the mobile site from the 
' main site home page. We expect to soon expand that to other types of mobile devices like iPhones, etc. 
USER_AGENT = Request.ServerVariables("HTTP_USER_AGENT")

ExtendedUserEmailEnquiryToSL = "User's Enquiry: " & UserEmailEnquiryToSL & " <br><br>User Agent: " & USER_AGENT   ' Append USER_AGENT to UserEmailEnquiryToSL so that the email will show
   											' ExtendedUserEmailEnquiryToSL in the Enquiry field.
If FALSE Then
    ...
    Response.Write "<br>UserEmailEnquiryToSL = " & UserEmailEnquiryToSL  
    Response.Write "<br>USER_AGENT = " 	 	 & USER_AGENT  
    Response.Write "<br>ExtendedUserEmailEnquiryToSL = " & ExtendedUserEmailEnquiryToSL & "<br><br>" 
End If 

UserEmailEnquiryToSL = ExtendedUserEmailEnquiryToSL	' So that the email will show ExtendedUserEmailEnquiryToSL in the Enquiry field.
...
If TRUE Then	
   ...
   'host = "mail.starlite-intl.com"             'The mail server name. (Commonly mail.yourdomain.xyz if your mail is hosted with HostMySite).
    host = "win-mail13.hostmanagement.net"
    ...
    ' [BN, 3/7/18] I changed password to the following (here and in file SCART/EmailSend.asp) ...
    'password = "S6a2n2I7"                       'Password for the above email address.
    ' [BN, 6/25/20] I changed password to the following (here and later also in files SCART/EmailSend.asp and Admin2/Emailing/EmailMergeINC.asp) ...
    'password = "!$2318Sz6No" 			        'Password for the above email address.
    'password = "$621zBn7!"
    'password  =  "x718Bnr#?_s"          'Changed Jan 26 2023
     password   = "T6#_S?By!zP"	      'Changed Jan 26 2023
    ...
    email_message = ""
    email_message = email_message & "<strong><font color='red'>Mobile</font> Customer:</strong>&nbsp;&nbsp;"
