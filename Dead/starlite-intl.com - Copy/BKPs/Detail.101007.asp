...
Sub MakeTable2(Direction)
	...
	For i = LBound(AccessoryID) to UBound(AccessoryID)
		...
		Else
			AccessorySQL = "Select PName, PID, ITEMID, PIC1, MSL, Duty, Cost, GPM, ShowPrice from PRODUCT WHERE ITEMID = '" & ItemID & "'"
    		...
    		If NOT rsAccessory.EOF Then  ' Sometimes an Accessory listed in HasAccessories column of Product table may not really be in the Product table.
				...
				If rsAccessory("PIC1") <> "notava1t.gif" Then
  					Response.Write "<td width=" & Col1Width & ">"
   					Response.Write "<a href=https://www.starlite-intl.com/Detail.asp?PID=" & rsAccessory("PID") & ">"
					Response.Write "<img border=0 width='30' src='imi/" & rsAccessory("PIC1") & "'"
    				Response.Write "</a>"
    				Response.Write "</td>"
				Else
					Response.Write "<td width=" & Col1Width & ">"
				...
				'Response.Write "<td><font size=1>" & "</font></td>"
  				...
  				Response.Write "<td align=left width=" & Col4Width & ">"
   				Response.Write "<a href=https://www.starlite-intl.com/Detail.asp?PID=" & rsAccessory("PID") & ">"
  				Response.Write rsAccessory("PName")
    			Response.Write "</a>"
    			Response.Write "</td>"
    			...
    			Response.Write "<td align=right width=" & Col5Width & ">"	' List Price ...
  				If Session("Country") = "USA" Then    
					...
					Response.Write formatcurrency(rsAccessory("MSL"))
				Else
					' 6/18/06, commented out, BN: Response.Write formatcurrency(((rsAccessory("MSL")*rsAccessory("Duty"))*RS("Freight"))*RS("Exch")) 
					Response.Write formatcurrency(rsAccessory("MSL")*rsAccessory("Duty")*RS("Exch")) 
				End If
    			Response.Write "</td>"
				...
				If Session("Country") = "USA" Then    
 					If rsAccessory("ShowPrice") = true Then			'  "ShowPrice" really means "Don't Show Price"  
						Response.Write "<font size=1>Click ORDER<br>to see price<br>or to order</font>"
					Else
						Response.Write "<font color=red><i>" & formatcurrency(rsAccessory("Cost")*RS("Freight")*(1/(1-(rsAccessory("GPM"))))) & "</i></font>"
					End If
				Else
					If CBool(rsAccessory("ShowPrice"))  Then			'  "ShowPrice" really means "Don't Show Price" 
						Response.Write "<font size=1>Click ORDER<br>to see price<br>or to order</font>"
					Else 
						Response.Write "<font color=red><i>" & formatcurrency(rsAccessory("Cost")*rsAccessory("Duty")*RS("Freight")*RS("Exch")*(1/(1-(rsAccessory("GPM"))))) & "</i></font>"
					End If
				End If
    			Response.Write "</td>"
  					
  				Response.Write "<td align=right>"  	
 				Response.Write "<a href=https://www.starlite-intl.com/scart/scart.asp?action=add&item=" & rsAccessory("ITEMID") & "&count=1&sid=0&Area=&sar=Special" & ">"
...
Sub MakeTable3(ItemID)
	...
	ParentsSQL = "Select PName, PID, ITEMID, PIC1, MSL, Duty, Cost, GPM, ShowPrice, HasAccessories from PRODUCT " & _
...
Sub OutputWarrantyRow(ItemID, Description)
	...
	WarrantyPID = rsWarrProducts("PID")
	WarrantyPic = rsWarrProducts("Pic1")
	WarrantyPrice = rsWarrProducts("Cost")*rsWarrProducts("Freight")*(1/(1-(rsWarrProducts("GPM")))) 
	If Session("Country") = "Canada" Then
		WarrantyPrice = WarrantyPrice * rsWarrProducts("Exch")
	End If
...
<meta name="keywords" content="GPS,GPS sensors,GPS sensor,GPS engine,GPS navigation,oem GPS,GPS system,car GPS,auto GPS,portable GPS,handheld GPS,marine GPS,GPS marine network,GPS receiver,GPS accessories,GPS antennas,fish finder,sounders,GPS software,GPS equipment,best GPS,pda GPS,Garmin GPS,bluetooth GPS,global positioning,tracking GPS,fleet tracking GPS,GPS antenna,USglobal GPS,discount GPS,GPS on sale,navigation electronics,CB radio,CB radios,CB,CB on sale,2-way radios,two way radios,walky talky,FRS,GMRS,marine radio,Uniden CB,Cobra CB,Midland CB,CB antennas,amateur radios,Galaxy radio,Magnum radio,radio scanner,radio scanners,scanner,flash memory,secure digital,compact flash,digital cameras,car audio,car stereos,car video,power amplifier,scanner antennas,power supplies,regulated power supplies,DJ equipment,accessories,hand tools,mechanics tools,MIT tools,Pyramid,Pyle,Silicon power,Solarcon,Fuji,Fujifilm,Nikon,Olympus,Panasonic,Motorola,Canon">
<meta name="description" content="Large selection of Garmin GPS, USglobal GPS, OEM GPS, GPS sensors, bluetooth GPS, fish finders, sounders, CB radios and walky-talky, flash memory, radio scanners, digital cameras, car audio and video, DJ, hand tools, mechanics tools">
