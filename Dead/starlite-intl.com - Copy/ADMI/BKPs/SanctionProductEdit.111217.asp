...
ElseIf Action = "SubmitNewProduct" Then	
	...
	'Response.Redirect "SanctionProductInsert.asp" 
	'End If    'msg = ""
...
Elseif  Action = "DeleteThisProduct" Then
	...
	Response.Redirect "SanctionProductEdit.asp"		' This just takes you back to where you were, to continue working ?
	'End If  'msg = ""
...
<head>
	<title>SanctionProductEdit.asp</title>
</head>
...
<% If Action = "EditOld" Then %>
<tr>
    <td>
    <font color="#FFBD00" size="2" face="Arial">
    First choose an existing product to edit. 
	</td>
</tr>
<% ElseIf Action = "Display Product to Edit" Then %>
<tr>
    <td>
    <font color="#FFBD00" size="2" face="Arial">
    Edit this product with the form below. 
	Then submit your changes using the button at top right or at bottom.
	Every product needs a numeric Item ID (containing no spaces).
	No two Item ID's should be the same.
	</td>
</tr>
<% ElseIf Action = "DeleteOld" Then %>
<tr>
    <td>
    <font color="#FFBD00" size="2" face="Arial">  
    First choose an existing product to delete. This will just display that product, not delete it. 
	</td>
</tr>
<% ElseIf Action = "Display Product to Delete" Then %>
<tr>
    <td>
    <font color="#FFBD00" size="2" face="Arial">
    Delete this product using the button at top right or at bottom.  
	</td>
</tr>
<% ElseIf Action = "CreateNew" Then %>
<tr>
    <td>
    <font color="#FFBD00" size="2" face="Arial">
    Enter a new product with the form below.  
	Then submit your data using the button at top right or at bottom.
	Every product needs a numeric Item ID (containing no spaces).
	No two Item ID's should be the same.
	</td>
</tr>
...
<tr>
	...
	<% If Action = "EditOld" OR Action = "DeleteOld" Then %>
	<tr>
		<% 
		If Action = "EditOld" Then 
			strLabel = "CHOOSE PRODUCT TO EDIT" 
		ElseIf Action = "DeleteOld" Then
			strLabel = "CHOOSE PRODUCT TO DELETE" 
		End If
		%>
		...
		<FORM Name="FORM1" ACTION="http://www.starlite-intl.com/Admi/SanctionProductEdit.asp?Action=<%=submitButtonValue%>" METHOD="GET" > 
			...
			<!-- <td  bgcolor="#bbbbbb"><INPUT type="submit" NAME="Action" VALUE="Edit"></td> -->
			<% 
			If Action = "EditOld" Then 
				submitButtonValue = "Display Product to Edit" 
			ElseIf Action = "DeleteOld" Then
				submitButtonValue = "Display Product to Delete" 
			End If
			%>
			<td  bgcolor="#bbbbbb"><INPUT type="submit" NAME="Action" VALUE="<%=submitButtonValue%>"></td>
	...
	Else				' When making a new product, so no PID exists yet.
		Val = ""		' This is the generic value to place in all fields of form2 when PID = "", i.e. when making a NEW product / record.
	End If
	%>


	<% 
	If (Action = "CreateNew") OR (Action = "Display Product to Edit") OR (Action = "Display Product to Delete") Then 
		If Action = "Display Product to Edit" Then
			NextAction			= "SubmitProductChanges" 
			txtNextActionButton = "Submit Product Changes"
		ElseIf Action = "Display Product to Delete" Then 
			NextAction			= "DeleteThisProduct" 
			txtNextActionButton = "Delete This Product"
		ElseIf Action = "CreateNew" Then
			NextAction			= "SubmitNewProduct"
			txtNextActionButton = "Submit New Product"
		End If
	%>

	<FORM Name="FORM2" ACTION="SanctionProductEdit.asp?Action=<%=NextAction%>&PID=<%=PID%>" METHOD=POST> 
	...
	<tr>
		...
		<td><INPUT type="submit"  NAME="Action" VALUE="<%=txtNextActionButton%>"></td>
	</tr>
	<% End If	' PID <> "" %>
	...
	<tr>
		...
		<% If Action = "CreateNew" Then %>
			<td><INPUT type="submit" NAME="Action" VALUE="<%=txtNextActionButton%>"></td>
		<% Else		' (Action = "Display Product to Edit") OR (Action = "Display Product to Delete") %>
			<td></td>
		<% End If %>
	</tr>
			...
			<select name="SID" size="1" >
				...
				Do While Not rsSubArea.EOF 
					If CStr(rsSubArea("SID")) = CStr(SID) Then %>
						<option value="<%=rsSubArea("SID")%>" selected="selected" > <%=rsSubArea("SID")%>&nbsp;&nbsp;<%=rsSubArea("Subname")%> </option>
				<%	Else %>
						<option value="<%=rsSubArea("SID")%>" > <%=rsSubArea("SID")%>&nbsp;&nbsp;<%=rsSubArea("Subname")%> </option>
				<%	End If 
				rsSubArea.MoveNext
				Loop
				   rsSubArea.Close %>
	...
	<tr>
		<td align=center bgcolor=bbbbbb colspan=3><INPUT type="submit" NAME="Action" VALUE="<%=txtNextActionButton%>" ></td>
	</tr>
...
<% End If   ' (Action = "CreateNew") OR (Action = "Display Product to Edit") OR (Action = "Display Product to Delete") %>


	<% If FALSE Then	' If Action = "DeleteOld" Then %>
		<!-- <FORM Name="form3" ACTION="SanctionProductInsert.asp" METHOD=POST>  -->
		<FORM Name="form3" ACTION="SanctionProductEdit.asp" METHOD="GET">       
		<tr>
			<td bgcolor="#790000"><font size="2" face="Arial" color="#FFFFFF"><b>PRODUCT PHYSICAL DELETE:<b></font></td>
